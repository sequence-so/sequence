...
const AudienceInspector = (props: Props) => {
  ...
  return (
    <InspectorSidebarBase
      icon={AudienceIcon}
      title={selectedAudience ? selectedAudience.name : ""}
      fillColor="#2074C5"
    ...
    >
      ...
      {data && (
        <>
          ...
          <CommonSelect
            containerWidth={"100%"}
            onChange={onChangeAudienceDropdown}
            value={selected}
            options={options}
          ></CommonSelect>
        </>
      )}
      {audienceLoading && <CircularProgress />}
      {audienceError && <GQLErrorMessage error={audienceError.message} />}
      {audienceNodeRef.current && audienceData && (
        <>
          <div className={styles.option_title}>AUDIENCE RULES</div>
          <AudienceBuilder
            renderTitle={false}
            ...
            editable={false}
          />
          <div className={styles.option_title}>MATCHED USERS</div>
          <ProductUserTable
            productUsers={audienceData.audiences.nodes[0].productUsers}
...
export default AudienceInspector;
